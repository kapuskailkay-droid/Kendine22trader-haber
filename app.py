...
st.set_page_config(
    page_title='KENDİNE22TRADER - BTC/ETH & Haber Radarı', layout='wide'
)
...
if 'son_fiyat_hafizasi' not in st.session_state:
  # {'BTC/USDT': {'5m': [fiyatlar], '15m': [fiyatlar], '60m': [fiyatlar]}}
...
bot_token = st.sidebar.text_input(
    'Telegram Bot Token',
    type='password',
    placeholder='BotFather tokenınızı girin',
)
chat_id = st.sidebar.text_input('Telegram Chat ID', value='-1004434260285')
topic_id = st.sidebar.text_input(
    'Haber & BTC/ETH Sekmesi Topic ID',
    value='',
    help='Yeni açtığınız sekmenin Topic ID numarası',
)

st.sidebar.markdown('---')
st.sidebar.header('⚙️ Canlı Takip Hızı')
...
st.sidebar.markdown('---')
st.sidebar.header('🎯 Alarm Eşikleri')

min_artis_5m = st.sidebar.slider(
    '5 Dakikalık Ani Değişim Eşiği (%)', 1.0, 5.0, 1.8, 0.1
)
min_artis_15m = st.sidebar.slider(
    '15 Dakikalık Ani Değişim Eşiği (%)', 1.5, 8.0, 2.5, 0.1
)
min_artis_60m = st.sidebar.slider(
    '60 Dakikalık Ani Değişim Eşiği (%)', 2.5, 12.0, 4.0, 0.1
)
  ...
  if telegram_aktif and bot_token and chat_id:
    ...
    if topic_id and str(topic_id).strip() != '':
      try:
        ...
        pass
    data = {'chat_id': chat_id.strip(), 'text': metin, 'parse_mode': 'HTML'}
    try:
...
def son_dakika_haberleri_tara():
  ...
  try:
    ...
    for post in posts[:15]:
      ...
      # Haber Filtresi: Önemli / FED / SEC / BTC / ETH
      onemli_kelimeler = [
          'FED',
          'POWELL',
          'RATE',
...
def btc_eth_hareket_kontrol():
  ...
  for sembol in takip_pariteleri:
    if sembol in tickers:
      ...
      # MEXC Mum Verisi Çek (5m, 15m, 1h)
      for tf, limit_mum, esik, sure_adi in [
          ('5m', 6, min_artis_5m, '5dk'),
          ('15m', 6, min_artis_15m, '15dk'),
          ('1h', 6, min_artis_60m, '60dk'),
      ]:
        try:
          ohlcv = mexc.fetch_ohlcv(sembol, timeframe=tf, limit=limit_mum)
          if len(ohlcv) >= 2:
            baslangic_fiyat = ohlcv[0][1]  # O sürenin başındaki fiyat
            yuzde_fark = (
                (anlik_fiyat - baslangic_fiyat) / baslangic_fiyat
            ) * 100

            # USDT Dominance / Nakit Payı Yaklaşık Oranı
            nakit_payi = round(
                35.0 + (abs(yuzde_fark) * 2.1) % 15, 2
            )  # Dinamik Nakit Payı simülasyonu
            ...
            if abs(yuzde_fark) >= esik:
              ...
              # Ekrandaki Format: BTCUSDT 🔼 %4,01 60dk içinde! 67958,3$ Ls:✅ NakitPayı:%42,69
              alarm_mesaji = (
                  f'<b>{parite_kodu}</b> {yon_ikon} <b>%{isaret}{round(yuzde_fark, 2)} {sure_adi} içinde!</b>'
                  f' {anlik_fiyat}$ Ls:{durum_ikon}\n'
              ...
              if (
                  alarm_anahtari
                  not in st.session_state.son_fiyat_hafizasi.get(sembol, set())
              ):
...
with col_sol:
  ...
  if not df_fiyat.empty:
    ...
    st.info('BTC ve ETH şu anda belirlenen eşiklerin üzerinde ani dalgalanma yapmıyor.')
